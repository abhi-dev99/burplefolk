...
def cmd_models(args: argparse.Namespace) -> None:
    models = app.ollama_get_models(args.ollama_endpoint)
    if not models:
...
def cmd_analyze(args: argparse.Namespace) -> None:
    ...
    analysis = app.run_analysis(source_type, file_map, args.profile_limit, db_config=db_cfg)
    if not analysis:
    ...
    if args.ai_brief:
        model = args.model
        if not model:
            available = app.ollama_get_models(args.ollama_endpoint)
            model = available[0] if available else "llama3:latest"
        print(f"\nGenerating AI brief with model: {model}")
        ai_brief = app.generate_ai_brief(analysis, model, args.ollama_endpoint)
        print("\n=== AI Brief ===")
        print(ai_brief)
